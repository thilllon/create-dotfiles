...
describe("findLatestCollection", () => {
  ...
  it("returns the newest collection folder by name, ignoring look-alikes", () => {
...
describe("restore", () => {
  ...
  it("rejects archives with an explanation instead of trying to read them", () => {
    createFile(home, `${newest}.zip`, "zip");
    createFile(home, `${newest}.tar.gz`, "tgz");

    expect(() => restore({ homeDir: home, source: join(home, `${newest}.zip`) })).toThrow(
      /archive is not supported/
    );
    expect(() => restore({ homeDir: home, source: join(home, `${newest}.tar.gz`) })).toThrow(
      DotfileError
    );
  });
  ...
  it("refuses to use the home directory itself as the source", () => {
    expect(() => restore({ homeDir: home, source: home })).toThrow(/not the home directory/);
  });
  ...
  it("records a failure when a destination cannot be written and continues", () => {
    ...
    expect(summary.failed[0].path).toBe(".zshrc");
    expect(summary.restored).toEqual([".config/nvim/lua/init.lua"]);
  });
});
